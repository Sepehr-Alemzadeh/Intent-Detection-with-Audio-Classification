...
def train(net, trainloader, optim, scheduler, criterion, epoch, device):
    ...
    print("Epoch: [{}]  loss: [{:.2f}] Accuracy [{:.2f}] ".format(epoch + 1, train_loss / len(trainloader), total_correct * 100 / total))
    
def validation(net, testloader, criterion, epoch, device, results_txt, model_name, val_acc, val_loss):
    ...
    if acc > best_acc:        
        ...
        if not os.path.isdir(save_point):
            os.mkdir(save_point)
        torch.save(state, save_point + model_name + str('_') + str(epoch) + '.t7')
        best_acc = acc
